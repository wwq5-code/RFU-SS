...
def unlearning_frkl(vibi_f_frkl, optimizer_frkl, vibi, epoch_test_acc, dataloader_erase,dataloader_remain, loss_fn,
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            if train_type == 'VIBU':
                loss = KLD_mean - BCE + args.unlearn_learning_rate * (kl_f_e - H_p_q)
            elif train_type == 'VIBU-SS':
                loss = KLD_mean - BCE + args.unlearn_learning_rate * (kl_f_e - H_p_q) + args.self_sharing_rate * (args.beta * KLD_mean2 + H_p_q2) # args.beta * KLD_mean - H_p_q + args.beta * KLD_mean2  + H_p_q2 #- log_z / e_log_py #-   # H_p_q + args.beta * KLD_mean2
...
def unlearning_frkl_train(vibi, dataloader_erase,dataloader_remain, loss_fn, reconstructor, reconstruction_function,
    ...
    if init_epoch == 0 or args.resume_training:
        ...
        vibi_f_frkl, optimizer_frkl, epoch_test_acc = unlearning_frkl(vibi_f_frkl, optimizer_frkl, vibi,
                                                                      epoch_test_acc, dataloader_erase,dataloader_remain, loss_fn,
...
def retraining_train(vibi, vibi_retrain, vibi_f_frkl,vibi_f_frkl_ss, dataloader_remain, dataloader_erase, reconstructor,
    ...
    if init_epoch == 0 or args.resume_training:
        ...
        print("KL_fr", np.mean(KL_fr), "KL_er", np.mean(KL_er), "KL_ssr", np.mean(KL_ssr), "KL_kl", np.mean(KL_kl))
        # print(KL_er)
...
def unlearning_main_body(args):
    ...
    vibi_retrain = retraining_train(vibi, vibi_retrain, vibi_f_frkl,vibi_f_frkl_ss, dataloader_remain, dataloader_erase, reconstructor,
                                    reconstruction_function,
                                    loss_fn, optimizer_retrain, test_loader, train_loader)
