...
def retraining_train(vibi, vibi_retrain, vibi_f_hessian, vibi_f_nipsu, vibi_f_frkl, vibi_f_frkl_ss, dataloader_remain,
                     dataloader_erase, reconstructor,
    ...
    if init_epoch == 0 or args.resume_training:
        ...
        for epoch in range(init_epoch, init_epoch + args.num_epochs):
            ...
            for step, (x, y) in enumerate(dataloader_remain, start=step_start):
                ...
                logits_z_e_nips, logits_y_e_nips, x_hat_e_nips, mu_e_nips, logvar_e_nips = vibi_f_nipsu(x,
                                                                                                        mode='forgetting')
        ...
        print("KL_fr", np.mean(KL_fr), "KL_hr", np.mean(KL_hr), "KL_nipsr", np.mean(KL_nipsr), "KL_er", np.mean(KL_er),
              "KL_ssr", np.mean(KL_ssr), "KL_kl", np.mean(KL_kl))
        # print(KL_er)
...
def unlearning_main_body(args):
    ...
    print()
    print("start NIPSU")
    vibi_f_nipsu, optimizer_nipsu = unlearning_frkl_train(copy.deepcopy(vibi).to(args.device), dataloader_erase,
                                                          dataloader_remain, loss_fn,
                                                          reconstructor,
    ...
    print()
    print("start VIBU")
    vibi_f_frkl, optimizer_frkl = unlearning_frkl_train(copy.deepcopy(vibi).to(args.device), dataloader_erase,
                                                        dataloader_remain, loss_fn,
                                                        reconstructor,
                                                        reconstruction_function, test_loader, train_loader,
                                                        train_type='VIBU')
    ...
    vibi_retrain = retraining_train(vibi, vibi_retrain, vibi_f_hessian, vibi_f_nipsu, vibi_f_frkl, vibi_f_frkl_ss,
                                    dataloader_remain, dataloader_erase, reconstructor,
                                    reconstruction_function,
                                    loss_fn, optimizer_retrain, test_loader, train_loader)
    ...
    vibi_f_hessian_w = vibi_f_hessian.state_dict()
    diff_grad = vibi_f_hessian.state_dict()
    for k in diff_grad.keys():
        diff_grad[k] = diff_grad[k] - diff_grad[k]
    retrain_net_w = vibi_retrain.state_dict()
    distance = 0
    for k in vibi_f_hessian_w.keys():
        diff_grad[k] = retrain_net_w[k] - vibi_f_hessian_w[k]
        distance += torch.norm(diff_grad[k].float(), p=2)
    print("retrain-hesian_unlearning_distance", distance)

    vibi_f_nipsu_w = vibi_f_nipsu.state_dict()
    diff_grad = vibi_f_nipsu.state_dict()
    for k in diff_grad.keys():
        diff_grad[k] = diff_grad[k] - diff_grad[k]
    retrain_net_w = vibi_retrain.state_dict()
    distance = 0
    for k in vibi_f_nipsu_w.keys():
        diff_grad[k] = retrain_net_w[k] - vibi_f_nipsu_w[k]
        distance += torch.norm(diff_grad[k].float(), p=2)
    print("retrain-nips_unlearning_distance", distance)
    ...
    for k in vibi_f_frkl_w.keys():
        ...
        distance += torch.norm(diff_grad[k].float(), p=2)
    print("retrain-vibu_unlearning_distance", distance)
