...
def init_vibi(dataset):
    ...
    if dataset == 'MNIST':
        ...
        approximator = LinearModel(n_feature=3 * 7 * 7, n_output=100)
        explainer = resnet18(3, 3 * 7 * 7 * 2)  # resnet18(1, 49*2)
        forgetter = LinearModel(n_feature=3 * 7 * 7, n_output=3 * 32 * 32)
        lr = 0.0001
...
def learning_train(dataset, model, step_start, loss_fn, reconstruction_function, optimizer):
    for step, (x, y) in enumerate(dataset, start=step_start):
        ...
        logits_z_log_softmax = logits_z.log_softmax(dim=1)
 #       print('y shape',y.shape)
        ...
        KL_z_r = (torch.exp(logits_z_log_softmax) * logits_z_log_softmax).sum(dim=1).mean() + math.log(
            logits_z_log_softmax.shape[1])

        x_hat = x_hat.view(x_hat.size(0), -1)
        x = x.view(x.size(0), -1)
        BCE = reconstruction_function(x_hat, x)  # mse loss
        loss = beta * KLD_mean + H_p_q #+ BCE / (batch_size * 32 * 32 * 3)
        ...
        if step % len(train_loader) % 500 == 0:
...
vibi, lr = init_vibi("CIFAR10")
vibi.to(device)

vibi_retrain, lr = init_vibi("CIFAR10")
...
if init_epoch == 0 or args.resume_training:
    ...
    for epoch in range(init_epoch, init_epoch + args.num_epochs):
        ...
        for step, (x, y) in enumerate(dataloader_erase, start=step_start):
            ...
            BCE = reconstruction_function(x_hat, x)  # mse loss
            loss = BCE / (batch_size * 32 * 32 * 3) * 10
...
vibi_f_temp, lr = init_vibi("CIFAR10")
...
# max_p, min_p = get_max_min_p(vibi_f_temp, dataloader_erase)
# p_range = max_p - min_p
...
if init_epoch == 0 or args.resume_training:
    ...
    for epoch in range(init_epoch, init_epoch + args.num_epochs):
        ...
        for step, (x, y) in enumerate(dataloader_erase, start=step_start):
            ...
            KLD_mean = 0.5 * torch.mean(
                logvar_f - logvar_e + (torch.exp(logvar_e) + (mu_e - mu_f).pow(2)) / torch.exp(
                    logvar_f) - 1).cuda() + 10
            ...
            logits_z_e = logits_z_e.view(logits_z_e.size(0), 3, 7, 7)
            ...
            e_log_p = torch.exp(BCE / (32 * 32 * 3 * batch_size))  # = 1/p
            e_log_py = torch.exp(H_p_q)
            log_z = torch.mean(logits_z_e.log_softmax(dim=1))
            loss = beta * KLD_mean - BCE / (32 * 32 * 3 * batch_size) - log_z / e_log_p  #
            ...
            if step % len(dataloader_erase) % 500 == 0:
    ...
    for epoch in range(init_epoch, init_epoch + args.num_epochs):
        ...
        for step, (x, y) in enumerate(dataloader_erase, start=step_start):
            x, y = x.to(device), y.to(device)  # (B, C, H, W), (B, 10)
            # x = x.view(x.size(0), -1)
            logits_z_e, logits_y_e, x_hat_e, mu_e, logvar_e = vibi(x, mode='forgetting')  # (B, C* h* w), (B, N, 10)

            logits_z_e = logits_z_e.view(logits_z_e.size(0), 3, 7, 7)
            x_hat_e = torch.sigmoid(reconstructor_for_unlearning(logits_z_e))
            x_hat_e = x_hat_e.view(x_hat_e.size(0), -1)
            x = x.view(x.size(0), -1)
            # x = torch.sigmoid(torch.relu(x))
            ...
            if epoch == args.num_epochs - 1:
                final_round_mse.append(BCE.item())
            if step % len(train_loader) % 5000 == 0:
                print("loss", loss.item())
...
if init_epoch == 0 or args.resume_training:
    ...
    for epoch in range(init_epoch, init_epoch + args.num_epochs):
        ...
        for step, (x, y) in enumerate(dataloader_remain, start=step_start):
            ...
            KLD_mean = torch.mean(KLD_element).mul_(-0.5).cuda()
            logits_z_r = logits_z_r.view(logits_z_r.size(0), 3, 7, 7)
            ...
            loss_r = beta * KLD_mean + H_p_q + BCE / (batch_size * 232 * 32 * 3)
            ...
            if step % len(dataloader_remain) % 2000 == 0:
    ...
    x_hat_r_cpu = x_hat_r_cpu.view(x_hat_r_cpu.size(0), 3, 32, 32)
