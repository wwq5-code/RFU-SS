...
def unlearning_frkl(vibi_f_frkl, optimizer_frkl, vibi, epoch_test_acc, dataloader_erase,dataloader_remain, loss_fn,
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            if train_type == 'VIBU':
                loss = args.kld_r *( KLD_mean - args.unlearn_bce_r * BCE) + args.unlearn_learning_rate * ( args.unlearn_ykl_r * kl_f_e - H_p_q)- args.reverse_rate * log_z
            elif train_type == 'VIBU-SS':
                loss = args.kld_r *( KLD_mean - args.unlearn_bce_r * BCE)  + args.unlearn_learning_rate * ( args.unlearn_ykl_r * kl_f_e - H_p_q) - args.reverse_rate * log_z + args.self_sharing_rate * (args.beta * KLD_mean2 + H_p_q2) # args.beta * KLD_mean - H_p_q + args.beta * KLD_mean2  + H_p_q2 #- log_z / e_log_py #-   # H_p_q + args.beta * KLD_mean2
...
if __name__ == '__main__':
    ...
    args.kld_r = 10
    args.self_sharing_rate = 0.0005
    args.unl_conver_r = 2
    args.hessian_rate = 13
