...
def learning_train(dataset, model, loss_fn, reconstruction_function, optimizer, args, epoch, mu_list,
                   sigma_list, train_loader, init_epoch):
    ...
    return model, optimizer, mu_list, sigma_list


def unlearning_frkl(vibi_f_frkl, optimizer_frkl, vibi, valid_acc, epoch_test_acc, dataloader_erase, loss_fn,
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (x, y) in enumerate(dataloader_erase, start=step_start):
            ...
            loss = args.beta * KLD_mean - BCE + kl_f_e - H_p_q #  #- log_z / e_log_py #-   # H_p_q + args.beta * KLD_mean2
...
def unlearning_frkl_train(vibi, valid_acc, dataloader_erase, loss_fn, reconstructor, reconstruction_function,
    ...
    if init_epoch == 0 or args.resume_training:
        ...
        vibi_f_frkl, optimizer_frkl, epoch_test_acc = unlearning_frkl(vibi_f_frkl, optimizer_frkl, vibi, valid_acc,
        ...
        for epoch in range(init_epoch, init_epoch + args.num_epochs):
            ...
            for step, (x, y) in enumerate(dataloader_erase, start=step_start):
                ...
                loss = BCE / (args.batch_size * 28 * 28) * 10
...
def retraining_train(vibi, vibi_retrain, dataloader_remain, vibi_f_frkl, reconstructor, reconstruction_function,
...
def unlearning_main_body(args):
    ...
    dataloader_full = DataLoader(full_set, batch_size=args.batch_size, shuffle=True)
    ...
    if init_epoch == 0 or args.resume_training:
        ...
        for epoch in range(init_epoch, init_epoch + args.num_epochs):
            ...
            vibi, optimizer, mu_list, sigma_list = learning_train(dataloader_full, vibi, loss_fn,
                                                                  reconstruction_function,
                                                                  optimizer, args, epoch, mu_list, sigma_list,
    ...
    vibi_f_frkl, optimizer_frkl = unlearning_frkl_train(vibi, valid_acc, dataloader_erase, loss_fn, reconstructor,
                                                        reconstruction_function, test_loader, train_loader)

    retraining_train(vibi, vibi_retrain, dataloader_remain, vibi_f_frkl, reconstructor, reconstruction_function,
                     loss_fn, optimizer_retrain, test_loader, train_loader)
